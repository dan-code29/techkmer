...
/* ------------------------------------------------------------------ */
/*  CATÉGORIES PRÉDÉFINIES (toujours affichées)                        */
/* ------------------------------------------------------------------ */
...
/* Associer un mot-clé (venant de la base) à une catégorie prédéfinie */
function matchCategory(apiCategory: string): string {
  ...
  if (lower.includes('network') || lower.includes('réseau') || lower.includes('reseau')) return 'Networking';
  ...
  if (lower.includes('access') || lower.includes('accès') || lower.includes('controle')) return 'Access Control';
  if (lower.includes('automation') || lower.includes('automatisme') || lower.includes('portail')) return 'Automation';
  if (lower.includes('accessoire') || lower.includes('outil') || lower.includes('câble') || lower.includes('cable') || lower.includes('mesure')) return 'Accessories';
  return apiCategory;
}
...
function ProductRow({
  ...
  return (
    <section className="mb-12">
      <div
        className={`${bgColor || ''} flex items-center justify-between mb-4 ${
          bgColor ? 'rounded-lg px-4 py-3' : ''
        }`}
      >
        <div>
          <h2
            className={`text-xl md:text-2xl font-bold flex items-center gap-2 ${
              bgColor ? 'text-white' : 'text-[#050B16]'
            }`}
          >
            {icon && <span>{icon}</span>}
            {title}
      ...
      <div className="relative">
        <button
          onClick={() => scroll('left')}
          className="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 z-10 w-10 h-10 rounded-full bg-white shadow-lg items-center justify-center text-[#050B16] hover:bg-[#0066FF] hover:text-white transition"
        ...
        <button
          ...
          className="hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 z-10 w-10 h-10 rounded-full bg-white shadow-lg items-center justify-center text-[#050B16] hover:bg-[#0066FF] hover:text-white transition"
          aria-label="Suivant"
        >
  ...
  const slides = [
    {
      ...
      desc: 'Éclairage intelligent, scénarios, contrôle à distance.',
      cta: 'Explorer la domotique',
      href: '#all-products',
      bg: 'from-indigo-900 via-purple-800 to-blue-700',
    },
    {
      badge: 'Sécurité',
      title: 'Protégez ce qui\ncompte vraiment',
      desc: 'Vidéosurveillance, alarmes, contrôle d\'accès.',
      cta: 'Voir la sécurité',
      href: '#all-products',
      bg: 'from-red-900 via-rose-800 to-red-700',
    },
    {
      badge: 'Énergie solaire',
      title: 'Autonomie\nénergétique',
      desc: 'Panneaux, batteries, onduleurs — installation incluse.',
      cta: 'Kits solaires',
      href: '#all-products',
  ...
  return (
    <div className={`relative h-[300px] md:h-[420px] rounded-lg overflow-hidden bg-gradient-to-br ${slide.bg}`}>
      ...
      <div className="relative z-10 h-full flex flex-col justify-center px-6 md:px-10 max-w-md">
        ...
        <h2 className="text-white text-2xl md:text-4xl font-bold mb-3 whitespace-pre-line leading-tight">
          {slide.title}
        </h2>
        <p className="text-gray-200 text-sm md:text-base mb-6 max-w-sm">{slide.desc}</p>
        <a
          href={slide.href}
          className="bg-white text-[#050B16] font-bold px-6 py-3 rounded-full w-fit hover:bg-[#00C2FF] hover:text-white transition shadow-lg text-sm"
        >
          {slide.cta}
        </a>
      </div>
      ...
      <button
        ...
        className="absolute left-2 top-1/2 -translate-y-1/2 z-10 w-9 h-9 rounded-full bg-black/40 hover:bg-black/60 text-white flex items-center justify-center backdrop-blur transition"
        aria-label="Précédent"
      >
      ...
      <button
        ...
        className="absolute right-2 top-1/2 -translate-y-1/2 z-10 w-9 h-9 rounded-full bg-black/40 hover:bg-black/60 text-white flex items-center justify-center backdrop-blur transition"
        aria-label="Suivant"
      >
...
function BoutiqueContent() {
  ...
  const [sortOption, setSortOption] = useState<string>(
    initialFilter === 'best' ? 'best-seller' : 'default'
  );
  ...
  /* Scroll vers la grille produits */
  ...
  const flashSales = useMemo(
    () => products.filter((p) => p.isPromotion === 1).slice(0, 10),
    [products]
  );
  const bestSellers = useMemo(
    () => [...products].sort((a, b) => (b.salesCount || 0) - (a.salesCount || 0)).slice(0, 10),
    [products]
  );
      ...
      {showAlert && (
        <div className="fixed top-24 right-4 bg-emerald-600 text-white px-5 py-3 rounded-lg shadow-2xl z-50 flex items-center gap-3">
          ...
          <span className="text-sm font-semibold">
            <strong>{lastProduct}</strong> ajouté au panier !
          </span>
        </div>
      )}
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 mb-6">
          ...
          <div className="lg:col-span-3 grid grid-cols-2 lg:grid-cols-1 gap-4">
            <a
              href="#all-products"
              className="rounded-lg p-5 bg-gradient-to-br from-emerald-600 to-emerald-500 text-white flex flex-col justify-between h-[150px] lg:h-[200px] shadow-md hover:shadow-lg transition group"
            >
              <span className="text-3xl group-hover:scale-110 transition-transform">🛠️</span>
              <div>
                <p className="text-[10px] uppercase tracking-wider opacity-90">Services pro</p>
                <p className="font-bold text-base leading-tight mt-1">
                  Devis gratuit<br />en 24h
                </p>
              </div>
            </a>
            <a
              href="#all-products"
              className="rounded-lg p-5 bg-gradient-to-br from-red-600 to-orange-500 text-white flex flex-col justify-between h-[150px] lg:h-[200px] shadow-md hover:shadow-lg transition group"
            >
              <span className="text-3xl group-hover:scale-110 transition-transform">🔥</span>
              <div>
                <p className="text-[10px] uppercase tracking-wider opacity-90">Offre flash</p>
                <p className="font-bold text-base leading-tight mt-1">
                  Jusqu&apos;à -50%<br />sur les kits
                </p>
              </div>
            </a>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-8">
          {TRUST_BADGES.map((badge) => {
            ...
            return (
              <div
                key={badge.title}
                className="bg-white rounded-lg border border-gray-100 p-3 flex items-center gap-3 hover:border-emerald-500 hover:shadow-md transition"
              >
            ...
            <div className="bg-gradient-to-r from-emerald-600 to-emerald-500 rounded-lg p-4 mb-4 flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className="bg-white text-emerald-700 text-xs font-bold px-4 py-2 rounded hover:bg-emerald-50 transition"
                >
                  Voir plus
                </button>
              </div>
        ...
        {/* ============================================ */}
        {/*  TOUS LES PRODUITS (avec ref pour scroll)    */}
        {/* ============================================ */}
        <section
          ref={allProductsRef}
          id="all-products"
          className="bg-white rounded-lg border border-gray-100 p-6 mb-8 scroll-mt-24"
        >
            ...
            <div className="hidden md:flex items-center gap-1 border border-gray-200 rounded-lg p-0.5">
              <button
                ...
                className={`w-8 h-8 rounded flex items-center justify-center transition ${
                  viewMode === 'grid' ? 'bg-emerald-600 text-white' : 'text-gray-500 hover:bg-gray-100'
                }`}
                aria-label="Grille"
              >
                <FaTh size={12} />
              ...
              <button
                ...
                className={`w-8 h-8 rounded flex items-center justify-center transition ${
                  viewMode === 'list' ? 'bg-emerald-600 text-white' : 'text-gray-500 hover:bg-gray-100'
                }`}
                aria-label="Liste"
              >
                <FaList size={12} />
            ...
            <div className="text-center py-16">
              ...
              {selectedCategory !== 'all' && (
                <button
                  ...
                  className="mt-4 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2 rounded-lg text-sm font-bold transition"
                >
          ...
          {itemsPerPage < filteredProducts.length && (
            <div className="text-center mt-8">
              <button
                ...
                className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-8 py-3 rounded-lg text-sm transition"
              >
          ...
          <div className="relative flex flex-wrap items-center justify-between gap-6">
            <div className="flex-1 min-w-[280px]">
              ...
              <h2 className="text-2xl md:text-3xl font-bold mb-2">
                Vous ne savez pas quel équipement choisir ?
              </h2>
              <p className="text-white/90 text-sm">
                Décrivez votre besoin et notre équipe vous proposera une solution adaptée.
              </p>
            </div>
            <div className="flex flex-wrap gap-3">
              <Link
                href="/devis"
                className="bg-white text-emerald-700 font-bold py-3 px-6 rounded-lg hover:bg-emerald-50 transition text-sm"
              >
                Demander conseil
              </Link>
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="bg-[#050B16] text-white font-bold py-3 px-6 rounded-lg hover:bg-black transition text-sm"
              >
                💬 WhatsApp
              </a>
            </div>
        ...
        {/* ============================================ */}
        {/*  NOS MARQUES & PARTENAIRES (tout en bas)     */}
        {/* ============================================ */}
        <section className="bg-white rounded-lg border border-gray-100 p-6 mb-8">
          <div className="text-center mb-6">
            <p className="text-emerald-600 text-xs font-bold uppercase tracking-[0.3em] mb-2">
              Nos partenaires
            </p>
            <h2 className="text-xl md:text-2xl font-bold text-[#050B16]">
            ...
            <p className="text-sm text-gray-500 mt-2">
              Nous travaillons avec les leaders mondiaux du matériel électrique, réseau et sécurité.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
            {BRANDS.map((brand) => (
              <div
                key={brand}
                className="flex items-center justify-center px-3 py-5 rounded-lg border border-gray-100 text-gray-500 font-bold text-xs md:text-sm text-center hover:border-emerald-500 hover:text-emerald-600 hover:shadow-md transition h-20"
              >
                {brand}
              </div>
            ))}
        ...
        <section className="rounded-2xl bg-gradient-to-r from-[#050B16] via-[#0a1a3a] to-[#050B16] p-8 md:p-12 text-center text-white relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <h2 className="text-2xl md:text-4xl font-bold mb-4">
              Nous commandons pour vous.
            </h2>
            <p className="text-gray-300 mb-8 max-w-2xl mx-auto">
              Vous ne trouvez pas un produit ? Contactez-nous, nous vous le procurons
              et l&apos;installons pour vous.
            </p>
