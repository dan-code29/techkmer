...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await turso.execute({
      sql: `INSERT INTO products (name, price, image, category, salesCount, dateAdded, description)
            VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING *`,
      args: [name, price, image, category, salesCount || 0, dateAdded, description || null],
    });
